...
class TelemetryService extends ChangeNotifier {
  ...
  static List<String> get _apiUrls {
    ...
    if (Platform.isAndroid) {
      return ['http://10.0.2.2/api/logs.php', 'http://127.0.0.1/api/logs.php', 'http://localhost/api/logs.php'];
    }
          ...
          if (jsonMap['status'] == 'success' && jsonMap['data'] is List) {
            ...
            for (var item in items) {
              final recordedAt = item['recorded_at'] != null 
                  ? DateTime.tryParse(item['recorded_at'].toString())?.millisecondsSinceEpoch ?? DateTime.now().millisecondsSinceEpoch
                  : DateTime.now().millisecondsSinceEpoch;

              fetchedLogs.add(SensorReading(
                id: 'log_${item['id'] ?? recordedAt}',
                ph: double.tryParse(item['ph']?.toString() ?? '7.2') ?? 7.2,
                ec: (double.tryParse(item['ec']?.toString() ?? '450') ?? 450) / 250.0, // normalized EC for UI
                oxygen: double.tryParse(item['oxygen']?.toString() ?? '8.2') ?? 8.2,
                waterTemp: double.tryParse(item['water_temp']?.toString() ?? item['temperature']?.toString() ?? '22.5') ?? 22.5,
                envTemp: double.tryParse(item['env_temp']?.toString() ?? item['temperature']?.toString() ?? '26.8') ?? 26.8,
                envHumidity: double.tryParse(item['env_humidity']?.toString() ?? item['humidity']?.toString() ?? '62.0') ?? 62.0,
                time: recordedAt,
              ));
            }
